...
#   - env "Windows NT x86" (nao "Windows x64")
#   - GhostscriptPath padrao usa %ALLUSERSPROFILE%\Application Data e gswin32c.exe
...
if (-not $GhostscriptPath) { $GhostscriptPath = "$env:ALLUSERSPROFILE\Application Data\Meddrive Printer\Ghostscript\bin\gswin32c.exe" }
...
[Win32CreateProfile]::EnumPrinterDrivers($null, "Windows NT x86", 1, [IntPtr]::Zero, 0, [ref]$needed, [ref]$returned) | Out-Null
$driverFound = $false
if ($needed -gt 0) {
    $buf = [System.Runtime.InteropServices.Marshal]::AllocHGlobal([int]$needed)
    try {
        if ([Win32CreateProfile]::EnumPrinterDrivers($null, "Windows NT x86", 1, $buf, $needed, [ref]$needed, [ref]$returned)) {
