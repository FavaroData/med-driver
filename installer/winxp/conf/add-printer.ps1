...
# Diferenca em relacao ao win7: env "Windows NT x86" (nao "Windows x64")
...
[Win32AddPrinter]::EnumPrinterDrivers($null, "Windows NT x86", 1, [IntPtr]::Zero, 0, [ref]$needed, [ref]$returned) | Out-Null
$driverFound = $false
if ($needed -gt 0) {
    $buf = [System.Runtime.InteropServices.Marshal]::AllocHGlobal([int]$needed)
    try {
        if ([Win32AddPrinter]::EnumPrinterDrivers($null, "Windows NT x86", 1, $buf, $needed, [ref]$needed, [ref]$returned)) {
