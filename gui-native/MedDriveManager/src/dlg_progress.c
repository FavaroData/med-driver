...
static void read_log(HWND hwnd, DWORD *pOffset, BOOL *pUtf16, BOOL *pBomChecked) {
    HANDLE hf = CreateFileW(PROGRESS_LOG, GENERIC_READ,
    ...
    if (*pUtf16) {
        /* Processa apenas pares completos de bytes (wchar_t) */
...
static DWORD WINAPI ps_thread(LPVOID param) {
    ...
    /* Monta parametros para powershell.exe */
    ...
    /* Aguarda o arquivo de log aparecer (ate 5s) */
    for (int i = 0; i < 50; i++) {
    ...
    /* Leitura incremental do log enquanto o processo roda */
    DWORD offset     = 0;
    BOOL  utf16      = FALSE;
    ...
    for (;;) {
        ...
        read_log(p->hwnd, &offset, &utf16, &bomChecked);
        if (done) break;
    }
    /* Drena o restante apos o processo encerrar */
    read_log(p->hwnd, &offset, &utf16, &bomChecked);
...
static INT_PTR CALLBACK ProgressDlgProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_INITDIALOG: {
        ...
        /* Botao Fechar: owner-draw + desabilitado ate o script terminar */
        ...
        p->hwnd = hwnd;
        HANDLE hThread = CreateThread(NULL, 0, ps_thread, p, 0, NULL);
    ...
    case WM_APP_PS_DONE: {
        ...
        /* Repinta o botao para refletir estado habilitado */
    ...
    case WM_CLOSE:
        /* Impede fechar enquanto o script esta rodando */
        if (s_done)
            EndDialog(hwnd, s_success ? IDOK : IDCANCEL);
        return 0;
...
BOOL dlg_progress_run(HWND parent,
    ...
    /* Localiza add-printer.ps1 na mesma pasta do EXE */
    wchar_t scriptPath[MAX_PATH];
    ...
    /* Aloca parametros para o thread (o thread libera) */
